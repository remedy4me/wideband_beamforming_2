...
%     thetai=theta_ii(iii);
    ...
    h_morm_restraint=1;
    for ii=1:M
        Tm(ii)=-floor(tau_sig(ii)/Ts+D)*Ts;
    end
    %% 构造理想数据协方差矩阵
    Bs=fu-fl;
    fc=(fu+fl)/2;
...
trap_dB_1=20*log10(abs(Beam_pb_1(:,find(theta==thetai))));
trap_dB_2=20*log10(abs(Beam_pb_2(:,find(theta==thetai))));

figure();
plot(Fpb,trap_dB_1,'b--o','MarkerFaceColor','k');
hold on
plot(Fpb,trap_dB_2,'k-o');
xlabel('归一化频率')
legend('旁瓣未控制','旁瓣控制')
ylabel('凹槽深度/dB')
ylim([-130,-60])
...
% figure();
% plot(theta_ii,SINR_out_noSL,'k-o','LineWidth', 1,'MarkerFaceColor','k','MarkerSize',5);
% hold on
% plot(theta_ii,SINR_out_2,'k-o','LineWidth', 1,'MarkerSize', 5);
% ylim([-25 15])
% legend('旁瓣未控制','旁瓣控制')
% ylabel('SINR(out)/dB')
% xlabel('\theta1(°)')